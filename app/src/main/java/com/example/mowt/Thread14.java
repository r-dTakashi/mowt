...
public class Thread14 extends AsyncTask<Uri.Builder, Void, String> {
	private final Activity MainActivity;

	public Thread14(Activity mainActivity) {
	...
	protected void onPostExecute(String result) {
		...
		if(in - out >= 7) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があり，上にもう2，3枚を着たり，ヒートテックを絶対着ましょう");
			else if(wind > 200) t.setText("風が強いので，上にもう2，3枚着たり，ヒートテックがおすすめ");
			else t.setText("もう3枚着るのがおすすめ");
		} else if(in - out >= 3) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があるため，もう2枚着るのがおすすめ");
			else if(wind > 200) t.setText("風が強いので，上にもう2枚着るのがおすすめ");
			else t.setText("外出時はもう1枚着ましょう");
		} else if(in - out >= -3){
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があります．外出時は気をつけましょう");
			else t.setText("外の温度は部屋と変わらず，温度変化も少ないので，おすすめはその服構成です");
		} else {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があるため，外出時は気をつけましょう");
			else t.setText("少し暑いかもしれないので，薄手の服がおすすめ");
		}
