...
public class Thread125 extends AsyncTask<Uri.Builder, Void, String> {
	private final Activity MainActivity;

	public Thread125(Activity mainActivity) {
	...
	protected void onPostExecute(String result) {
		...
		if(in - out >= 7) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があり，上にもう3枚を着たり，ヒートテック，長ズボンがおすすめ");
			else if(wind > 200) t.setText("風が強いので，上にもう3枚着たり，ヒートテック，長ズボンがおすすめ");
			else t.setText("もう3枚着たり，ヒートテック，長ズボンがおすすめ");
		} else if(in - out >= 3) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があるため，もう2,3枚着たり，長ズボンがおすすめ");
			else if(wind > 200) t.setText("風が強いので，上にもう2,3枚着たり，長ズボンがおすすめ");
			else t.setText("もう2,3枚着たり，長ズボンがおすすめ");
		} else if(in - out >= -3){
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があります．外出時は気をつけましょう");
			else t.setText("外の温度は部屋と変わらず，温度変化も少ないので，その服構成がおすすめ");
		} else {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化がありますが，その服構成がおすすめ");
