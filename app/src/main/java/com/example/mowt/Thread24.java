...
public class Thread24 extends AsyncTask<Uri.Builder, Void, String> {
	private final Activity MainActivity;

	public Thread24(Activity mainActivity) {
	...
	protected void onPostExecute(String result) {
		...
		if(in - out >= 7) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があります．上にもう2,3枚+コートを着るのがおすすめ");
			else if(wind > 200) t.setText("風が強いので，上にもう2,3枚+コートを着るのがおすすめ");
			else t.setText("上にもう2枚+コートを着るのがおすすめ");
		} else if(in - out >= 3) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があります．上にもう1,2枚+アウターを着るのがおすすめ");
			else if(wind > 200) t.setText("風が強いので，上にもう1,2枚+アウターを着るのがおすすめ");
			else t.setText("上にもう1枚+アウターを着たほうがいいかも");
		} else if(in - out >= -3){
			if(out - p_out < 2) t.setText("今日は急激な温度変化があります．ジャケットを着てもいいかも");
			else t.setText("外の温度は部屋と変わらず，温度変化も少ないので，その服構成がおすすめ");
		} else {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化がありますが，少し暑いかもしれないので気をつけてください");
			else t.setText("少し暑いかもしれないです．気をつけてください");
		}
