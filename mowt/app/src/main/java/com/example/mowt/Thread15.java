...
public class Thread15 extends AsyncTask<Uri.Builder, Void, String> {
	private final Activity MainActivity;

	public Thread15(Activity mainActivity) {
	...
	protected void onPostExecute(String result) {
		...
		if(in - out >= 7) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があり，上にもう3，4枚を着たり，ヒートテック，長ズボンがおすすめ");
			else if(wind > 200) t.setText("風が強いので，上にもう3，4枚着たり，ヒートテック，長ズボンがおすすめ");
			else t.setText("もう3，4枚着たり，ヒートテック，長ズボンがおすすめ");
		} else if(in - out >= 3) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があるため，もう3枚着たり，長ズボンがおすすめ");
			else if(wind > 200) t.setText("風が強いので，上にもう3枚着たり，長ズボンがおすすめ");
			else t.setText("もう3枚着たり，長ズボンがおすすめ");
		} else if(in - out >= -3){
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があります．ロングTシャツを着てもいいかも");
