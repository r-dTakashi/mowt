...
public class Thread234 extends AsyncTask<Uri.Builder, Void, String> {
	private final Activity MainActivity;

	public Thread234(Activity mainActivity) {
	...
	protected void onPostExecute(String result) {
		...
		if(in - out >= 7) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があります．厚着の中にもう2枚，厚着をコートにしてもいいかも");
			else if(wind > 200) t.setText("風が強いので，厚着の中にもう2枚着て，厚着をコートにしてもいいかも");
			else t.setText("厚着の中にもう1,2枚着て，厚着をコートにしてもいいかも");
		} else if(in - out >= 3) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があります．厚着の中にもう1枚着て，厚着をコートにしてもいいかも");
			else if(wind > 200) t.setText("風が強いので，厚着の中にもう1枚着て，厚着をコートにしてもいいかも");
			else t.setText("もう1,2枚着たほうがいいかも");
		} else if(in - out >= -3){
			if(out - p_out < 2) t.setText("今日は急激な温度変化があります．中に服を着てもいいかも");
			else t.setText("外の温度は部屋と変わらず，温度変化も少ないので，その服構成がおすすめ");
		} else {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化がありますが，少し暑いかもしれないので半袖+長袖がおすすめ");
			else t.setText("少し暑いかもしれないです．厚着がいらないかもしれません");
		}
