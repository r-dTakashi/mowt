...
public class Thread1234 extends AsyncTask<Uri.Builder, Void, String> {
	private final Activity MainActivity;

	public Thread1234(Activity mainActivity) {
	...
	protected void onPostExecute(String result) {
		...
		if(in - out >= 7) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があり，上にもう1，2枚着たり，ヒートテックがおすすめ");
			else if(wind > 200) t.setText("風が強いので，上にもう1，2枚着たり，ヒートテックがおすすめ");
			else t.setText("もう1，2枚着るのがおすすめ");
		} else if(in - out >= 3) {
			if(Math.abs(out - p_out) > 2) t.setText("今日は急激な温度変化があるため，もう1枚着るのがおすすめ");
			else t.setText("もう1枚着るのがおすすめ");
		} else if(in - out >= -3){
			...
			else t.setText("少し暑いかもしれないので，1枚脱いでもいいかもしれません");
		}
	}
